...
static void buildgophermapline(char *linebuff, int linebuff_len, char itemtype, char *desc, char *selector, char *server, long port, char *curdirectory, struct MotsognirConfig *config) {
  char *itemserver = server;
...
static void sendbackgopherplushack(int sock, struct MotsognirConfig *config) {
  char txtline[1024];
...
static long execCgi(int sock, char *localfile, char **srvsideparams, struct MotsognirConfig *config, char *version, char *scriptname, char *remoteclientaddr, char *launcher, int gophermapflag) {
  char tmpstring[4096];
  char *cmd;
  int res;
...
static int requestlookslikegopherplus(char *req) {
...
static int checkforevasion(char *gopherroot, char **pubdirlist, char *localfile) {
